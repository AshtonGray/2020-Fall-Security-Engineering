...
class des():
    ...
    def substitute(self, d_e):  # Substitute bytes using SBOX
        ...
        round = 0
        for i in subblocks:
            bin = self.compute_s_box(i, round)
            for j in bin:
                result.append(int(j))
            round += 1
    ...
    def compute_s_box(self,block,round):
        ...
        row = [int(block[0]), int(block[5])]
        column = [int(block[1]), int(block[2]), int(block[3]), int(block[4])]

        rowdec = row[0]*2 + row[1]*1
        columndec = column[0]*8 + column[1]*4 + column[2]*2 + column[3]*1

        bin = S_BOX[round][rowdec][columndec]
        bin = binvalue(bin, 4)
        return bin
